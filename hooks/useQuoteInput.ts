...
export function useQuoteInput(targetText: string) {
  ...
  const processInput = useCallback((currentValue: string, previousValue: string, currentStates: DisplayCharState[]): QuoteInputState => {
    ...
    for (let i = 0; i < targetLength; i++) {
     ...
     if (currentLength === targetLength && newStates[targetLength - 1]?.state === 'current') {
         ...
         newStates[targetLength - 1].state = lastTypedChar === lastTargetChar ? 'correct' : 'incorrect';
    ...
    if (isComplete && newStates[targetLength] && newStates[targetLength].state === 'current') {
  ...
  }, [targetText, state.backspaceCount]); // Include dependencies
  ...
  const handleInput = useCallback((event: Event) => {
    ...
    setState(prevState => processInput(newValue, prevState.inputValue, prevState.charStates));
